...
- **`ArcadeState`** — Self-contained retro side-scrolling minigame ("Stagecoach Stampede"). Owns its own Three.js scene/camera. Jump/duck to avoid obstacles, collect trust documents for Fiduciary Points. Pushed from exploration and popped on game-over via `onEnd(score)` callback.
...
- `ClientGenerator.js` — Procedurally generates Reception-room clients: picks type (`Retiree`, `Entrepreneur`, etc.), rolls assets/fees, assigns 0–2 positive and 0–2 negative attributes, and scales enemy combat stats to asset size. Exports `generateClient(overrideLastName?)`, `generateBeneficiaryChain()`, `applyChainModifiers()`, `calculatePortfolioHealth()`, `POSITIVE_ATTRIBUTES`, `NEGATIVE_ATTRIBUTES`.
- `bestiary.js` — `BESTIARY_DATA` keyed by visual ID; tracks defeated enemy entries with flavor quips.
- `thoughts.js` — `ROOM_THOUGHTS` keyed by room ID; Andrew's inner monologue shown on first room visit.
...
- **`TouchControls`** — Virtual d-pad + action buttons for mobile. Injects directly into `InputManager.keys` so all game logic works unchanged. Only activates on touch-capable devices.
...
- **Beneficiary chains**: `generateBeneficiaryChain()` creates a 3-client family sharing a last name. Use `applyChainModifiers(client, chain)` after each decision to make followers friendlier (accepted) or angrier (rejected). Only `chainEligible` client types appear in chains.
- **Portfolio health**: `calculatePortfolioHealth(clients, aum, fees)` returns `{ rating, score, grade, avgAUM, feeYield }` for end-of-run scoring.
