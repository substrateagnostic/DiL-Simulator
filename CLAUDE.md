...
- **`SaveManager`** — localStorage persistence. Auto-save fires after every room transition and after story combat victories (not reception clients).
...
- **`MenuState`** — Pause/inventory menu. Tabs: Abilities, Cosmetics, Journal (enemy log), Achievements, Save Game. "Journal" not "Bestiary".
- **`ArcadeState`** — Self-contained retro minigame. Owns its own Three.js scene.

### Combat System (`src/combat/`)

- **`CombatEngine`** — Pure logic (no rendering). Key methods: `playerAttack()`, `playerAbility(id)`, `playerItem(id)`, `playerBrace()`, `playerPowerMove()`, `playerPressAdvantage()`, `playerSecondWind()`, `playerRetaliate()`, `playerFlee()`, `enemyTurn()`, `processTurnStart(who)`, `telegraph()`, `getActivePhaseIndex()`.
...
- **Brace**: `playerBrace()` sets `player.bracing = true`, adds DEF+5 for 2 turns. The next incoming `attack` or `summon` hit is halved, clears the flag, and sets `player.retaliateReady = true`.
- **Retaliate**: `playerRetaliate()` — free counter-attack after a successful brace (22 base power, +15 momentum). Only available when `retaliateReady` is true.
...
**`CombatHUD.showMainMenu` signature**: `showMainMenu(silenced, momentum, bracing, retaliateReady)` — pass all four. Tiered momentum buttons and Retaliate are injected dynamically.

**XP bar**: `_updateStats()` reads `stats._xpTable` (pass `_xpTable: XP_TABLE` in the stats object) to render a progress bar. Uses `XP_TABLE[level - 1]` as the start of the current level and `XP_TABLE[level]` as the next threshold.
...
- `ClientGenerator.js` — Procedurally generates Reception-room clients. `give_item` actions use the `item` field (not `itemId`).
...
- **`Player`** — Holds `stats` (including `aum`), `inventory`, `flags`, `questStates`, `position`, `currentRoom`, `actIndex`, `upgradePoints`, `unlockedAbilities`. Key methods: `gainXP()`, `rest()`, `addItem()/useItem()`, `setFlag()/getFlag()`, `serialize()/deserialize()`. Emits `flag-set` on EventBus. `gainXP()` caps at `XP_TABLE.length` (15).
- **`NPC`** — Per-NPC `interactRange` option (defaults to `PLAYER.INTERACT_RANGE`). `conditionFn` toggled each frame by `EntityManager`.
...
- **`CombatHUD`** — `showMainMenu(silenced, momentum, bracing, retaliateReady)`. Renders Brace, Retaliate (when ready), tiered momentum buttons (25/50/100), telegraph row, XP bar, and Confidence bar. `updateTelegraph(hint)` sets the enemy intent text. `showTaunt(text, side)` renders speech bubbles on `'player'` or `'enemy'` side.
...
- **Combat result flow**: `onEnd(result)` returns `'victory'`, `'defeat'`, or `'flee'`. On defeat, `retry_<encounterId>` flag is set; the next encounter routes to a `<encounterId>_retry` dialog. `ending_started` is reset on defeat so the executive floor boss re-triggers.
- **NPC conditional spawning**: set `condition: { flag: 'x' }` or `condition: { notFlag: 'x' }` in the room data NPC entry. NPCs with any condition initialize hidden; no extra code needed.
...
- **Auto-save**: `ExplorationState._autoSave()` calls `SaveManager.save(player.serialize())`. Fires silently on every room transition, with a toast only on story combat victories.
