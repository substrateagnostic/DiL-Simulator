...
const writeReport = (result) => {
  ...
  L.push('| `meredith_boss` combat A/B best pair 0.62% / 12,891 px, unexplained | 0.62% unexplained | **cross-build best pair 0.55% (11,422 px) vs SAME-BUILD best pair 0.55% (11,426 px)** | **CLOSED — it is animation phase, not look.** The two agree to 4 px. `karen`\'s same-build floor is 0.61% while its cross-build number is 0%, which is the same effect from the other side. |');
