...
# VENDOR PATCH (TRUST ISSUES charmetrics, 2026-07-31): the verdict line below prints
# U+2192 (→). Windows consoles default stdout to the cp1252 codepage, which cannot
# encode that character, so an un-patched run dies with
# UnicodeEncodeError instead of printing the verdict. Force UTF-8 on stdout/stderr at
# import time rather than requiring callers to set PYTHONIOENCODING/PYTHONUTF8 env
# vars. Guarded: reconfigure() is Python 3.7+ and only exists on real TextIOWrapper
# streams, so this is a no-op (never a crash) when stdout has already been swapped
# for something else (e.g. captured by a test harness).
