...
const head = execSync('git show HEAD:src/data/dialogs/index.js', { maxBuffer: 1 << 28 }).toString();
...
console.log(bad ? `\nFAIL - ${bad}` : '\nTEXT-ONLY PASS - node count, node types and every next/ifTrue/ifFalse are byte-identical to HEAD in all three trees');
