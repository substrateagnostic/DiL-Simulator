// THROWAWAY research fetcher for the gender-of-performance study.
// Same wire protocol and same armature-only extractor as tools/meshy-clip-fetch.mjs,
// but every byte it writes lands under art/char_refs/meshy_pilot/_clips/gender/
// so the shipped clip library in public/meshy/clips/ is never touched (another
// lane owns src/ and the git index this pass).
//
//   node tools/_gender-fetch.mjs --ids=247,251 [--rig=<rig_task_id>] [--tag=andrew]
//
// Raw GLBs  -> _clips/gender/raw/<tag>_action_<id>.glb
// Clip GLBs -> _clips/gender/clips/<tag>_a<id>.glb
// Ledger    -> _clips/gender/spend_gender.json  (per-task consumed_credits)
import { readFileSync, writeFileSync, mkdirSync, existsSync, statSync } from 'fs';
import { join } from 'path';

const ENV_PATH = 'C:/Users/agall/projects/un_party_game/.env';
const REPO = 'C:/Users/agall/projects/DiL_Simulator';
const GDIR = join(REPO, 'art/char_refs/meshy_pilot/_clips/gender');
const RAW = join(GDIR, 'raw');
const OUT = join(GDIR, 'clips');
const BASE = 'https://api.meshy.ai/openapi/v1';
const LEDGER = join(GDIR, 'spend_gender.json');

const args = Object.fromEntries(process.argv.slice(2).map(a => {
  const m = a.match(/^--([^=]+)(?:=(.*))?$/); return m ? [m[1], m[2] ?? true] : [a, true];
}));
const ACTION_IDS = String(args.ids || '').split(',').filter(Boolean).map(Number);
const TAG = args.tag || 'andrew';
// andrew's rig is the donor the whole shipped clip library was authored on.
const RIGS = {
  andrew: '019fbbef-72f0-7ace-9d64-1fb2e6355b0c',
  karen: '019fbc03-c6f4-7d33-bcc8-9457fedad140',
};
const RIG = args.rig || RIGS[TAG];
if (!ACTION_IDS.length) { console.error('need --ids'); process.exit(1); }
if (!RIG) { console.error('need --rig or a known --tag'); process.exit(1); }
...
// ── GLB helpers (verbatim from tools/meshy-clip-fetch.mjs) ──────────────────
...
function extractClip(srcBuf, actionId) {
  ...
  const parts = []; let offset = 0; const bufferViews = [];
  ...
  return writeGLB({
    asset: { version: '2.0', generator: 'trust-issues _gender-fetch' },
    scene: 0, scenes: json.scenes, nodes: json.nodes,
    accessors, bufferViews, buffers: [{ byteLength: offset }], animations,
  }, Buffer.concat(parts));
}

// ── main ────────────────────────────────────────────────────────────────────
log(`rig ${TAG} = ${RIG.slice(0, 8)}…  ids=${ACTION_IDS.join(',')}`);
const ledger = existsSync(LEDGER) ? JSON.parse(readFileSync(LEDGER, 'utf8')) : { tasks: {} };
let spent = 0;

for (const actionId of ACTION_IDS) {
  const rawPath = join(RAW, `${TAG}_action_${actionId}.glb`);
  if (!existsSync(rawPath)) {
    const res = await api('POST', '/animations', { rig_task_id: RIG, action_id: actionId });
    const taskId = res.result;
    log(`${TAG} a${actionId}: task ${taskId}`);
    const r = await poll(`/animations/${taskId}`, `a${actionId}`);
    const url = r.animation_glb_url || r.glb_url;
    if (!url) throw new Error(`no glb url for a${actionId}: ${JSON.stringify(r).slice(0, 300)}`);
    ...
    ledger.tasks[`${TAG}_${actionId}`] = { task: taskId, action_id: actionId, rig: TAG, consumed_credits: credits };
    spent += credits;
    writeFileSync(LEDGER, JSON.stringify(ledger, null, 2));
    log(`${TAG} a${actionId}: ${(statSync(rawPath).size / 1048576).toFixed(2)}MB, ${credits} cr`);
  } else {
    log(`${TAG} a${actionId}: raw already on disk, no spend`);
  }
  const clip = extractClip(readFileSync(rawPath), actionId);
  writeFileSync(join(OUT, `${TAG}_a${actionId}.glb`), clip);
  log(`${TAG} a${actionId}: clip -> ${(clip.length / 1024).toFixed(1)} KB`);
}
